...
class Account(unittest.TestCase):
    ...
    def test_quickPay(self,bank):
        ...
        if bank=='邮储银行':
            ...
            self.driver.find_element_by_name("请输入验证码").send_keys('1234')
    ...
    def test_transPass(self):
        WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located((By.NAME,'6~20位字符，至少包含数字、字母、符号中的两种，不可设置与登录密码相同')))
        views=self.driver.find_elements_by_class_name('android.widget.EditText')
        views[0].send_keys('a12345')
        views[1].send_keys('a12345')
        self.driver.find_element_by_name('确定').click()
    ...
    def test_recharge(self):
        ...
        views[1].send_keys(Common(self.driver).getVercode(GVariable.teleNum)[-6:])
        #self.driver.hide_keyboard()
    ...
    def test_accountInfo(self):
        WebDriverWait(self.driver, 40).until(EC.visibility_of_element_located((By.NAME,'安全退出')))
        ...
        CMethod().assertText(text[14].text,"已设置")
